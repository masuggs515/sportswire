...
| 2026-03-22 | Game detail page | task/mlb-highlights | New /scores/{league} and /scores/{league}/{gameId} routes. GameDetailClient: sticky sub-header (back + live badge + share), three collapsible sections (Overview with 30s live auto-refresh, Box Score with team tabs + lazy ESPN summary fetch per-sport, Highlights MLB-only lazy). ScoresClient: inline box score expansion removed; all cards navigate to game detail. NavBar: Scores active on all /scores/* paths. |
